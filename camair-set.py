...
def setcam(sock, opcode, preset, value):
    bs = bytes(value, 'utf-8')
    values = (0x00, len(value)+2, opcode, preset-1, bs)
    packer = struct.Struct('bbbb%ds' % (len(value),))
...
def main(args):
    ...
    if args.set_name is not None:
        if setcam(sock, 0x0b, args.preset, args.set_name):
            print("Setting preset {:d} name SUCCESSFUL".format(preset))
        else:
            print("Setting preset {:d} name FAILED".format(preset))


    if args.set_stream is not None:
        if setcam(sock, 0x09, args.preset, args.set_stream):
            print("Setting preset {:d} stream url SUCCESSFUL".format(preset))
        else:
            print("Setting preset {:d} stream url FAILED".format(preset))

    if args.set_logo is not None:
        if setcam(sock, 0x0d, args.preset, args.set_logo):
            print("Setting preset {:d} logo url SUCCESSFUL".format(preset))
        else:
            print("Setting preset {:d} logo url FAILED".format(preset))
